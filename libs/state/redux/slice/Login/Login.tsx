...
export const getLogoutUser = createAsyncThunk(
  ...
  async ({ authManager }: { authManager: IAuthManager }, thunkAPI) => {
    try {
...
export const changePassword = createAsyncThunk(
  ...
  async ({ api, body }: { api: IAuthAPI; body: ChangePasswordReq}, thunkAPI) => {
    try {
      const res: ChangePasswordRes = await api.changePassword(body)
      return { ...res }
    } catch (error) {
      return thunkAPI.rejectWithValue(error)
    }
