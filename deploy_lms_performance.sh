...
URL=https://lms-performance.vercel.app/
...
{
  "projectId":"prj_Du8LYPP1tdzGonYw9IyLwxv2V3hL",
...
if vercel deploy --prod; then
  MESSAGE="Deployed sapp-lms-performance success to: $URL at $(date '+%Y-%m-%d %H:%M:%S')"
else
  MESSAGE="Deployment FAILED for sapp-lms-performance at $(date '+%Y-%m-%d %H:%M:%S')"
fi
