...
function FormRequest({ open, setOpen, reloadPage }: IProps) {
  const { userContextApi, myRequestApi, dispatch, useAppSelector, router, pathname, query } = useFeature()
  ...
  const useFormProp = useForm<IMyRequest>({
  ...
  const onSubmit = async (data: IMyRequest) => {
