...
interface isNotificationIconInterface extends IIcon {
  isNotifications: boolean
}
const NotificationIcon = ({
  className,
  isNotifications,
}: isNotificationIconInterface) => {
  const socket = useContext(SocketContext)
  const [countUnread, setCountUnread] = useState(0)

  useEffect(() => {
    if (socket && socket.connected) {
      socket.on('GET_NOTIFICATION_UNREAD', (data) => {
        setCountUnread(data.data.total_records)
      })
    }
  ...
  return (
    <>
      ...
      {countUnread > 0 && isNotifications && (
        <span className="absolute -top-1.5 -right-1.5 flex items-center justify-center h-4 w-4 rounded-full bg-danger text-white text-ssm">
          {countUnread}
        </span>
      )}
      {countUnread === 0 && isNotifications && (
