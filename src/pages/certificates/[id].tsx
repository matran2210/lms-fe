 ...
 */
  const useGetData = (queryKey: string, params: Object) => {
    const fetchData = async () => {
      const { data } = await CoursesAPI.getCertificate(
        router.query.id,
      )
      return data
    }
      ...
      <div className='h-full'>
        <div className='bg-white h-[70px] flex justify-between items-center px-6'>
          <div className='flex items-center'>
            <div className='text-bw-1 text-lg-xl font-medium me-3'>{certificate?.course?.name}</div>
            <div className='text-gray-1 text-sm me-1'>Issued by</div>
            <div className='text-primary text-sm font-medium hover:underline cursor-pointer' onClick={() => window.open('https://sapp.edu.vn', '_blank')}>SAPP Academy</div>
          </div>
          <div className='flex'>
            {/* <div className='cursor-pointer'>Share</div> */}
            <ClickToCopyButton link={`${process.env.NEXT_PUBLIC_WEB_LMS_URL}/certificates/${certificate?.id}`}>
              <SappButtonIcon title='Share' isBgPrimary isTextPrimary>
                <ShareLinkIcon />
              </SappButtonIcon>
            </ClickToCopyButton>

            <SappButtonIcon title='Download' className='ms-4' download link={certificate?.certificate_url} target='_blank' onClick={() => download()}>
              <IconDownload />
            </SappButtonIcon>
          </div>
        </div>
        <div className='bg-gray-3 flex justify-center items-center md:min-h-[94vh] lg:min-h-[95vh] 3xl:min-h-[93vh] max-h-[100vh] min-h-[95vh]'>
          <Image src={certificate?.certificate_url} alt={certificate?.course?.name} className='object-contain' width={1100} height={800} priority />
        </div>
      </div>
    </SappLoadingGlobal>
  )
