...
import { CLASS_STATUS, CLASS_USER_TYPES, COURSE_STATUS, BUTTON_STATUS } from 'src/constants';
...
export const useCourseStatus = (course: ICourse) => {
  ...
  const percentProgress =
    round(
      (Number(student?.learning_progress?.total_course_sections_completed ?? 0) /
        Number(student?.learning_progress?.total_course_sections ?? 0)) * 100,
      2
    ) || 0;
  ...
  const checkStatusCourse = () => {
    ...
    if (courseStatus === COURSE_STATUS.PUBLISH || courseStatus === COURSE_STATUS.LOCK) {
      if (classUserType === CLASS_USER_TYPES.TRANSFERED_TO && classInstance?.type === 'LESSON') {
        return BUTTON_STATUS.Disabled;
      }

      if (classStatus === CLASS_STATUS.PUBLIC || classStatus === CLASS_STATUS.ENDED) {
        if (course?.course_type === 'TRIAL_COURSE' && !student) {
          if (classInstance?.duration_type === 'FLEXIBLE') return BUTTON_STATUS.Active;

          if (classInstance?.duration_type === 'FIXED' && classInstance?.finished_at) {
        ...
        if (startedAt && finishedAt) {
          ...
          if (course?.course_type === 'TRIAL_COURSE' && finishedAtDate <= formattedDate)
            return BUTTON_STATUS.Extend;

          if (finishedAtDate <= formattedDate) return BUTTON_STATUS.Disabled;

          if (finishedAtDate > formattedDate) {
            if (studentStatus === CLASS_USER_STATUS.READY_TO_LEARN) return BUTTON_STATUS.Begin;
            if (studentStatus === CLASS_USER_STATUS.IN_PROGRESS) return BUTTON_STATUS.Resume;
            if (studentStatus === CLASS_USER_STATUS.COMPLETED) return BUTTON_STATUS.Review;
          } else return BUTTON_STATUS.Disabled;
        }
      ...
      if (classStatus === CLASS_STATUS.DRAFT || classStatus === CLASS_STATUS.BLOCK)
