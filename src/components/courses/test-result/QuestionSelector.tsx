...
import { MultipleQuestionProps } from 'src/type/courses-3-level/test'
...
export default function QuestionSelector({
  ...
  const renderBoxes = (type: string, data: IAnswer[], totalBefore: number) => {
    const renderBoxItems = data?.map((item: IAnswer, index: number) => {
      return (
        <button
          key={item?.id}
          onClick={() => {
            router.push(`/short-course/explanation/${item?.id}?title=My Course`)
          }}
          disabled={
  ...
  const renderLines = (type: string, data: IAnswer[], totalBefore: number) => {
    const renderBoxItems = data?.map((item: IAnswer, index: number) => {
      return (
        <button
          key={item?.id}
          disabled={
            questions?.quizAttempt?.status === EAttemptStatus.UN_SUBMITTED ||
            !item.id
          }
          onClick={() => {
            if (
              ...
              router.push(
                `/short-course/explanation/${item?.id}?title=My Course`,
              )
            }
          }}
