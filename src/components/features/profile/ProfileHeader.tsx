...
const ProfileHeader = ({ isEdit, setAvatar }: IProps) => {
  const { user, loading, loadingEditName, loadingEditAvatar } =
    useAppSelector(userReducer)

  const [reViewImageSrc, setReViewImageSrc] = useState<string>()
  ...
  const validateImage = (type: string): boolean => {
    ...
    <div className="shadow-box  block pr-8.25 py-10 pl-10 bg-white mb-6">
      <div className="lg:flex block justify-start items-center gap-6">
        <div className="relative">
          <div className="w-30 pt-29.2 rounded-full border-2 w border-primary  shrink mb-6 lg:mb-0 ">
            <div
              className={`${
            ...
            >
              ...
              <Image
                src={
                  reViewImageSrc || user.detail.avatar['150x150'] || blankAvatar
                }
                alt="avatar"
          ...
          {isEdit && (
            <div
              className="absolute right-0 bottom-2 w-fit opacity-1 z-50"
