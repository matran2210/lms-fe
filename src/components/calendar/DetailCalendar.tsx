...
const DetailCalendar = ({ open, setOpen }: IProps) => {
  ...
  const getKeyContent = () => {
    return data?.key_before_contents?.map((item) => {
      return (
        <div
          key={item.id}
          className="bg-gray-4 text-gray-14 max-w-[111px] px-2 py-1 text-sm"
        >
          {item.name}
        </div>
      )
  ...
  return (
    ...
    >
      <div>
        <div className="border-gray-2 border border-solid px-7 py-4">
        ...
        {!(data?.is_case_study || data?.schedule?.is_holiday) &&
          !isOnlyMidTermOrFinalTest && (
            <div className="border-gray-2 mt-4 border border-solid px-7 py-4">
