...
const Course = ({ course }: { course: any }) => {
  ...
  const showStatus = course?.status === CLASS_USER_STATUS.READY_TO_LEARN ? 'Ready to learn' : course?.status === CLASS_USER_STATUS.COMPLETED ? 'Completed' : course?.status === CLASS_USER_STATUS.IN_PROGRESS ? 'In progress' : 'Expired'
  const showTextButton = course?.status === CLASS_USER_STATUS.READY_TO_LEARN ? 'Active' : course?.status === CLASS_USER_STATUS.COMPLETED ? 'Review' : course?.status === CLASS_USER_STATUS.IN_PROGRESS ? 'Resume' : (course?.status === CLASS_USER_STATUS.CANCELED && course?.course_type === 'TRIAL_COURSE') ? 'Extend' : ''

  return (
    <div className="cursor-pointer">
      <div
        className={`name-course text-2xl font-semibold mb-4 xl:h-[60px] ${course?.status === CLASS_USER_STATUS.CANCELED ? 'text-gray-2' : 'text-bw-1'}`}
        onClick={() => course.status !== CLASS_USER_STATUS.CANCELED ? router.push(`/courses/my-course/${course.id}`) : {}}
      >
      ...
      <div className="des mt-6 mb-8">
        <p
          ...
          className={`text-bas h-20  ${course?.status !== CLASS_USER_STATUS.CANCELED ? 'text-bw-1' : 'text-gray-1 '}`}
        />
      </div>
            ...
            <div className="progress mb-6 h-8">
              <div className="info flex justify-between mb-2">
                <div className="text flex items-baseline">
                  <Icon type={course?.status === CLASS_USER_STATUS.READY_TO_LEARN ? 'like' : course.status === CLASS_USER_STATUS.IN_PROGRESS ? 'hour' : ''} className="relative top-0.5" />
