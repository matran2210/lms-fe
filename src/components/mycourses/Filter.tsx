...
const Filter= ({courses}:any) => {
  ...
  const handleChange = (selected: any) => {
    ...
    router.push(`/courses?name=${router.query.name??''}&type=${selected.label}`)
  };
  ...
  return (
    <div className="result filter flex">
      ...
      <HookFormSelect
        options={courses?.total?.map((category:any) => ({ label: category?.categoryName, value: category?.categoryName }))}
