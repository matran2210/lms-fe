...
const EventRepeatField = ({
  ...
  useEffect(() => {
    const subscription = watch((value, { name, type }) => {
      if (value?.repeat_type?.value === EVENT_REPEAT_TYPES.NO_REPEAT)
        return onChange()

      const getInterval = () => {
        if (value?.repeat_type?.value === EVENT_REPEAT_TYPES.CUSTOM)
          return value?.repeat_frequency?.interval || 1

        return 1
      }
      const getFrequency = () => {
        switch (value?.repeat_type?.value) {
      ...
      const getDayOfWeek = () => {
        if (
          value?.repeat_type?.value === EVENT_REPEAT_TYPES.CUSTOM &&
          value?.repeat_frequency?.unit === FREQUENCY_UNITS.WEEK
        )
          return mapRepeatOn(value?.repeat_on)

        if (value?.repeat_type?.value === EVENT_REPEAT_TYPES.EVERY_WEEKDAY)
          return mapRepeatOn(
            REPEAT_ON.filter((day) => day !== 'T7' && day !== 'CN'),
          )

        if (value?.repeat_type?.value === EVENT_REPEAT_TYPES.WEEKLY)
      ...
      const getDayOfMonth = () => {
        if (
          value?.repeat_type?.value === EVENT_REPEAT_TYPES.MONTHLY ||
          (value?.repeat_type?.value === EVENT_REPEAT_TYPES.EVERY_WEEKDAY &&
            value?.repeat_frequency?.unit === FREQUENCY_UNITS.MONTH) ||
          value?.repeat_type?.value === EVENT_REPEAT_TYPES.ANNUALLY
        )
      ...
      const getMonthOfYear = () => {
        if (value?.repeat_type?.value === EVENT_REPEAT_TYPES.ANNUALLY)
  ...
  const is_repeat = repeat_type.value !== EVENT_REPEAT_TYPES.NO_REPEAT
  const is_custom_repeat = repeat_type.value === EVENT_REPEAT_TYPES.CUSTOM
  const repeat_on_visible =
    repeat_type.value === EVENT_REPEAT_TYPES.CUSTOM &&
  ...
  return (
    <>
      ...
      <div className={`event-repeat ${className}`}>
        ...
        />
        {is_repeat && (
          <div className="mt-2 grid grid-cols-repeat-label gap-y-6 rounded-none border border-[#DBDFE9] px-[15px] py-5">
            {is_custom_repeat && (
              <>
            ...
            <Controller
              ...
              render={({ field }) => (
                <DatePicker
                  ...
                  maxDate={dayjs(field.value).add(2, 'year')}
