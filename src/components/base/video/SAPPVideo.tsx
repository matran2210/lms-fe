...
const SAPPVideo = ({
  ...
  function skipAhead(event: Event) {
    ...
    streamRef.current.currentTime = parseInt(skipTo)
    if (progressBarRef?.current) {
  ...
  const getThumbnail = () => {
    ...
    return {
      src:
        thumbnail?.['1031x605'] ?? '/assets/images/default_thumbnail_video.png',
      width: 1031,
      height: 605,
    }
  }
  ...
  return (
    <>
      <div
        className={`${loading ? 'visible' : 'hidden'} flex-center relative h-full w-full overflow-hidden rounded-lg pt-[56.26%]`}
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 top-0 h-full w-full rounded-lg bg-overlay-loading opacity-80 transition-opacity">
          <LoadingIcon
            loading={loading}
          ...
          {cloudflarePlayer ? (
            ...
            >
              ...
              <video
                ...
                controls={false}
                className={`${styles.content}`}
                poster={src ?? '/assets/images/default_thumbnail_video.png'}
                onSeeking={() => {
              ...
              />
              <div
                className="video-controls flex-center absolute bottom-0 left-0 right-0 hidden h-14 w-full rounded-b-lg px-4 py-3"
