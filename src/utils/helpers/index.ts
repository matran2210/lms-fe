...
export function isMobileExtensive() {
  const toMatch = [
    ...
    /avantgo/i,
    /bada\/|blackberry|blazer/i,
    /compal|elaine|fennec|hiptop|iemobile/i,
    /ip(hone|od)/i,
    /iris|kindle|lge /i,
    /maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i/i,
    /palm( os)?|phone/i,
    /p(ixi|re)\/|plucker|pocket/i,
    /psp|series(4|6)0|symbian|treo/i,
    /up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i,
    /1207|6310|6590|3gso|4thp|50[1-6]i|770s/i,
    /802s|a wa|abac|ac(er|oo|s\-)/i,
    /ai(ko|rn)|al(av|ca|co)|amoi/i,
    /an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)/i,
    /attw|au(di|\-m|r |s )|avan/i,
    /be(ck|ll|nq)|bi(lb|rd)/i,
    /bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)/i,
    /c55\/|capi|ccwa|cdm\-|cell/i,
    /chtm|cldc|cmd\-|co(mp|nd)/i,
    /craw|da(it|ll|ng)|dbte|dc\-s|devi/i,
    /dica|dmob|do(c|p)o|ds(12|\-d)/i,
    /el(49|ai)|em(l2|ul)|er(ic|k0)/i,
    /esl8|ez([4-7]0|os|wa|ze)/i,
    /fetc|fly(\-|_)/i,
    /g1 u|g560|gene|gf\-5|g\-mo/i,
    /go(\.w|od)|gr(ad|un)/i,
    /haie|hcit|hd\-(m|p|t)|hei\-/i,
    /hi(pt|ta)|hp( i|ip)/i,
    /hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)/i,
    /hu(aw|tc)|i\-(20|go|ma)/i,
    /i230|iac( |\-|\/)|ibro|idea/i,
    /ig01|ikom|im1k|inno/i,
    /ipaq|iris|ja(t|v)a|jbro/i,
    /jemu|jigs|kddi|keji|kgt( |\/)/i,
    /klon|kpt |kwc\-|kyo(c|k)/i,
    /le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])/i,
    /libw|lynx|m1\-w|m3ga|m50\//i,
    /ma(te|ui|xo)|mc(01|21|ca)/i,
    /m\-cr|me(rc|ri)|mi(o8|oa|ts)/i,
    /mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)/i,
    /mt(50|p1|v )|mwbp|mywa/i,
    /n10[0-2]|n20[2-3]|n30(0|2)/i,
    /n50(0|2|5)|n7(0(0|1)|10)/i,
    /ne((c|m)\-|on|tf|wf|wg|wt)/i,
    /nok(6|i)|nzph|o2im/i,
    /op(ti|wv)|oran|owg1/i,
    /p800|pan(a|d|t)|pdxg/i,
    /pg(13|\-([1-8]|c))|phil|pire/i,
    /pl(ay|uc)|pn\-2|po(ck|rt|se)/i,
    /prox|psio|pt\-g|qa\-a/i,
    /qc(07|12|21|32|60|\-[2-7]|i\-)/i,
    /qtek|r380|r600|raks|rim9/i,
    /ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)/i,
    /sc(01|h\-|oo|p\-)|sdk\//i,
    /se(c(\-|0|1)|47|mc|nd|ri)|sgh\-/i,
    /shar|sie(\-|m)|sk\-0/i,
    /sl(45|id)|sm(al|ar|b3|it|t5)/i,
    /so(ft|ny)|sp(01|h\-|v\-|v )/i,
    /sy(01|mb)|t2(18|50)|t6(00|10|18)/i,
    /ta(gt|lk)|tcl\-|tdg\-/i,
    /tel(i|m)|tim\-|t\-mo/i,
    /to(pl|sh)|ts(70|m\-|m3|m5)/i,
    /tx\-9|up(\.b|g1|si)/i,
    /utst|v400|v750|veri|vi(rg|te)/i,
    /vk(40|5[0-3]|\-v)|vm40|voda|vulc/i,
    /vx(52|53|60|61|70|80|81|83|85|98)/i,
    /w3c(\-| )|webc|whit|wi(g |nc|nw)/i,
    /wmlb|wonu|x700|yas\-/i,
    /your|zeto|zte\-/i,
  ]
